...
export default function UploadPage() {
      ...
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="flex justify-center gap-3 mt-3">
            <Link href={`/login?redirect=${encodeURIComponent(window.location.pathname)}`} className="underline">
              Đăng nhập
            </Link>
            <Link href={`/register?redirect=${encodeURIComponent(window.location.pathname)}`} className="underline">
              Đăng ký
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-8">
      ...
      {/* form tạo truyện */}
      <NovelForm user={user} onSuccess={(novelId) => router.push(`/novels/${novelId}`)} />
    </div>
  );
}
